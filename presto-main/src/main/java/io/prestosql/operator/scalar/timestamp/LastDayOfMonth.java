...
public class LastDayOfMonth
{
    ...
    public static long lastDayOfMonth(ConnectorSession session, @SqlType("timestamp(p)") long timestamp)
    {
        long epochMillis = scaleEpochMicrosToMillis(timestamp);
        long millis = getChronologyFromSession(session).monthOfYear().roundCeiling(epochMillis + 1) - MILLISECONDS_IN_DAY;
    ...
    {
        return lastDayOfMonth(session, timestamp.getEpochMicros());
    }
}
